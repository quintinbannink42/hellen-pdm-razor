...
## This commit — remaining 9 IS long-haul + EN B re-geometry

| Item | Status |
|------|--------|
| DRC shorts | **1** (J2 VBAT↔GND only — unchanged) |
| DRC crossings | **87 → 119** after B.Cu south-ring fill (IS westbound vs ADIO/PWR verticals; EN thin saved ~4). F-corridor cut is in `scripts/is_f_west_corridor.py`. |
| Unconnected (DRC) | **122 → 115** |
| EN/IS unconnected | **9 → 0** (all 12 EN + all 12 IS long-haul closed) |
| Tracks / vias | **340 tracks + 106 vias** (was 286 / 104) |
...
|--------|------------------------|-------------|
| DRC shorts | **1** | **1** |
| DRC crossings | **87** | **119** |
| DRC unconnected | **122** | **115** |
...
| tracks_crossing (**119**) | Up from 87 — 9 IS B.Cu south-ring highways vs ADIO/PWR verticals (same tradeoff as AUX4). EN re-geometry saved ~4. |
...
| 5 | create-board / copper finish | **Partial** — EN + all 12 IS long-haul closed; crossings still high; GND islands remain |

## Remaining polish

- Cut IS/EN crossings without reopening EN/IS (F westbound + B west corridor; see `scripts/is_f_west_corridor.py`)
...
- `scripts/route_is_longhaul_thin_en.py` — remaining 9 IS B south-ring + EN B thin (this commit)
- `scripts/is_f_west_corridor.py` — F westbound + B west-corridor crossing cut
